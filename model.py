...
with tf.device('/cpu:0'): #device:GPU:1
    ...
    with tf.Session(config=config) as sess:
        model = keras.models.load_model('program/07-fpn-triangle-120.hdf5', custom_objects={'tversky_loss': tversky_loss, 'iou_loss_core': iou_loss_core})
        #sess.run(tf.global_variables_initializer())
        pred = model.predict(x_test, verbose=0)
        pred = np.argmax(pred,axis=3).astype(int)
        final_pred = np.ndarray((pred.shape[0], pred.shape[1] * 4, pred.shape[2] * 4))
        ...
        for i in range(len(filenames)): 
            ...
            imsave(impath, final_pred[i].astype(int))
